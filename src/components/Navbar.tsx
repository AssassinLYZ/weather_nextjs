      ...
      <nav className="shadow-sm  sticky top-0 left-0 z-50 bg-white">
        <div className="h-[80px]     w-full    flex   justify-between items-center  max-w-7xl px-3 mx-auto">
          <div className="flex items-center justify-center gap-2  ">
            <p className=" text-cyan-600 text-3xl">WEATHER SEARCH</p>
            <MdWbSunny className="text-3xl mt-1 text-orange-400" />
          </div>
          {/*  */}
          <section className="flex gap-2 items-center">
            ...
            />
            <MdOutlineLocationOn className="text-3xl text-cyan-600" />
      ...
      {((showSuggestions && suggestions.length > 1) || error) && (
        <ul className="mb-4 bg-white absolute border top-[44px] left-0 border-gray-300 rounded-md min-w-[200px]  flex flex-col gap-1 py-2 px-2">
          {error && suggestions.length < 1 && (
            <li className="text-red-500/60 p-2 font-bold "> {error}</li>
          )}
